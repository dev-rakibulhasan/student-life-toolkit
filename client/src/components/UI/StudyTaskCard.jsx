import React from "react";
import moment from "moment";
import { formatTime } from "../../Utils";
const StudyTaskCard = ({ task, onEdit, onToggle, onDelete }) => {
  const priorityColors = {
    low: "badge-info",
    medium: "badge-warning",
    high: "badge-error",
  };

  const isOverdue = !task.completed && new Date(task.deadline) < new Date();

  return (
    <div
      className={`card bg-base-100 shadow hover:shadow-lg transition-shadow ${
        task.completed ? "opacity-70" : ""
      }`}
    >
      <div className="card-body">
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={task.completed}
              onChange={onToggle}
              className="checkbox checkbox-primary"
            />
            <h3
              className={`card-title ${task.completed ? "line-through" : ""}`}
            >
              {task.title}
            </h3>
          </div>
          <div className="flex gap-1">
            <button
              className="btn btn-sm btn-circle btn-outline btn-primary"
              onClick={onEdit}
              title="Edit Task"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-4 w-4"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"
                />
              </svg>
            </button>
            <button
              className="btn btn-sm btn-circle btn-outline btn-error"
              onClick={onDelete}
              title="Delete Task"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-4 w-4"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"
                />
              </svg>
            </button>
          </div>
        </div>

        <div className="flex flex-wrap gap-2 mb-3">
          <span className="badge badge-outline">{task.subject}</span>
          {task.topic && (
            <span className="badge badge-outline">{task.topic}</span>
          )}
          <span className={`badge ${priorityColors[task.priority]} capitalize`}>
            {task.priority}
          </span>
          {isOverdue && <span className="badge badge-error">Overdue</span>}
          {task.completed ? (
            <span className="badge badge-success">Completed</span>
          ) : (
            <span className="badge badge-warning">Pending</span>
          )}
        </div>

        {task.description && <p className="text-sm mb-3">{task.description}</p>}

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            <p>
              <strong>Deadline:</strong> {moment(task.deadline).format("llll")}
            </p>
            <p>
              <strong>Estimated Time:</strong> {task.estimatedHours} hours
            </p>
          </div>

          {task.timeSlots.length > 0 && (
            <div>
              <p>
                <strong>Scheduled Times:</strong>
              </p>
              <div className="text-xs">
                {task.timeSlots.map((slot, index) => (
                  <span key={index} className="badge badge-ghost mr-1 mb-1">
                    {slot.day}: {formatTime(slot.startTime)}-
                    {formatTime(slot.endTime)}
                  </span>
                ))}
              </div>
            </div>
          )}
        </div>

        {task.completedAt && (
          <p className="text-xs text-gray-500 mt-2">
            Completed on: {moment(task.completedAt).format("llll")}
          </p>
        )}
      </div>
    </div>
  );
};

export default StudyTaskCard;
